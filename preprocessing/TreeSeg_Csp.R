...
#  - Resume-safe: skips already completed trees
#  - Voxel_size fixed to 0.3 (5cm was causing dimension crash)
#  - quantile(probs=0.05) for robust Z estimation
#  - unname() prevents "5%" label from breaking seed_map
# ============================================================
...
############################################
# 5️⃣  MULTI-TREE SEGMENTS — CSP + split per tree
############################################
cat("--- Processing multi-tree segments (CSP re-segmentation) ---\n")
flush.console()
...
for (i in seq_len(nrow(multi_segs))) {
  ...
  # Unlist and validate coordinates
  ...
  if (length(xs) == 0) {
    cat(sprintf("  [%d/%d] pred_%s — SKIP (no valid seeds)\n",
  ...
  # ── Resume: skip if all output trees already exist ───────
  ...
  cat(sprintf("    Loaded %d points\n", lidR::npoints(las_seg)))
  flush.console()
  
  # ── Build seed map (X, Y, Z, TreeID) ─────────────────────
  # Z = 5th percentile of points within 0.5m of trunk + 1.3m (DBH)
  # unname() strips the "5%" label that breaks data.frame construction
  z_vec <- sapply(seq_along(xs), function(j) {
    ...
    unname(quantile(las_seg@data$Z[nearby], probs = 0.05))
  })
  ...
  seed_map <- data.frame(
    ...
    Y      = as.numeric(ys),
    Z      = as.numeric(z_vec),
    TreeID = as.integer(seq_along(xs))
  )
  ...
  if (sum(in_bounds) == 0) {
    cat(sprintf("    SKIP — all seeds outside cloud extent\n"))
    cat(sprintf("    Cloud: X[%.1f,%.1f] Y[%.1f,%.1f]\n", x_min, x_max, y_min, y_max))
    cat(sprintf("    Seeds: X[%.1f,%.1f] Y[%.1f,%.1f]\n",
                min(xs), max(xs), min(ys), max(ys)))
    flush.console()
    for (j in seq_along(ids)) {
      file.copy(laz_path,
                file.path(out_folder, paste0(ids[j], ".laz")),
                overwrite = TRUE)
    }
    results_log[[i]] <- list(predinstance = pred_id,
                             status       = "seeds_out_of_bounds",
  ...
  if (sum(in_bounds) < nrow(seed_map)) {
    cat(sprintf("    Warning: %d of %d seeds out of bounds — dropping\n",
                sum(!in_bounds), nrow(seed_map)))
    flush.console()
    seed_map <- seed_map[in_bounds, ]
    ids      <- ids[in_bounds]
  }
  
  cat(sprintf("    Seed map: %d trees\n", nrow(seed_map)))
  flush.console()
  ...
  seg_result <- tryCatch({
    ...
    las_geom <- CspStandSegmentation::add_geometry(las_seg, n_cores = 4)
    
    cat("    Running csp_cost_segmentation...\n"); flush.console()
    
    CspStandSegmentation::csp_cost_segmentation(
      las_geom,
      seed_map,
      Voxel_size = 0.3,    # DO NOT lower this — 0.05 causes dimension crash
      V_w        = 0.5,
      N_cores    = 4
    )
  ...
  # ── CSP failed: copy original for each tree ──────────────
  if (is.null(seg_result)) {
    cat("    FALLBACK — copying original unsplit points for each tree\n")
    flush.console()
    for (j in seq_along(ids)) {
      file.copy(laz_path,
                file.path(out_folder, paste0(ids[j], ".laz")),
    ...
    results_log[[i]] <- list(predinstance = pred_id,
                             status       = "csp_failed_kept_original",
                             n_trees_in   = length(xs),
                             n_trees_out  = NA)
    next
  }
  
  # ── Split by TreeID — one LAZ per tree ───────────────────
  ...
  for (t in tree_ids_found) {
    ...
    # TreeID 1,2,3 maps back to ids[1], ids[2], ids[3]
    tree_label <- if (t <= length(ids)) ids[t] else
      paste0("pred", pred_id, "_tree", t)
    
    out_path <- file.path(out_folder, paste0(tree_label, ".laz"))
    lidR::writeLAS(las_tree, out_path)
    
    cat(sprintf("    Saved %s — %d points\n",
                basename(out_path), lidR::npoints(las_tree)))
    flush.console()
    n_saved <- n_saved + 1
  }
  
  # Report unassigned points
  n_unassigned <- sum(seg_result@data$TreeID == 0, na.rm = TRUE)
  if (n_unassigned > 0) {
    cat(sprintf("    Note: %d unassigned points (TreeID=0) discarded\n", n_unassigned))
    flush.console()
  }
...
skipped_count <- sum(log_dt$status == "skipped_already_exists",   na.rm = TRUE)
...
oob_count     <- sum(log_dt$status == "seeds_out_of_bounds",      na.rm = TRUE)
invalid_count <- sum(log_dt$status == "invalid_seeds",            na.rm = TRUE)

cat(sprintf("Single-tree segments copied:    %d\n", single_ok))
cat(sprintf("Multi-tree re-segmented (ok):   %d\n", ok_count))
cat(sprintf("Multi-tree already done:        %d\n", skipped_count))
cat(sprintf("Multi-tree fallback (CSP fail): %d\n", failed_count))
cat(sprintf("Seeds out of bounds:            %d\n", oob_count))
cat(sprintf("Files not found:                %d\n", missing_count))
cat(sprintf("Empty segments skipped:         %d\n", empty_count))
cat(sprintf("Invalid seeds skipped:          %d\n", invalid_count))
...
# Save processing log
log_path <- file.path(out_folder, "_resegmentation_log.csv")
fwrite(log_dt, log_path)
cat(sprintf("Processing log: %s\n", log_path))
